...
// 베타 피드백(불편·개선 의견) 분류 기준 — QA(버그)와 별개 트랙.
// 버그가 아니라 "불편해요 / 이랬으면 좋겠어요" 류를 가볍게 받아 분류한다.
export const FEEDBACK_GUIDE = `
## 피드백 유형 (feedbackType)
- ux      : 불편함·헷갈림·동선/UI 개선 의견 (버그 아님 — "이게 불편해요")
- request : 새 기능·옵션 추가 요청 ("~기능 있으면 좋겠어요")
- content : 문구·번역·콘텐츠·데이터 내용 개선
- etc     : 위에 안 맞는 일반 의견

## 서비스/기능 영역
- 위 [라우팅 맵]의 "서비스 맵"·"기능 영역"을 그대로 쓴다. 모르면 service=unknown, feature 빈칸.

## 피드백 필수 데이터 — 없으면 enough=false 로 1~2개만 되묻는다
1. 화면/기능 — 어느 화면·기능에 대한 의견인가
2. 불편/개선 내용 — 무엇이 불편한지 / 무엇을 원하는지
   · 둘 다 자명하면 enough=true.
- ⚠️ 피드백은 기종(iOS/Android)·재현 절차·심각도를 묻지 않는다.
`.trim();
